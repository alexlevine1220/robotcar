...
""" Old signal checking method """
# def signalCheck(rectCoordinates, rectWidth, rectHeight, point1, point2, x, y):
#     distances = [(1001, -1, -1), (1001, -1, -1), (1001, -1, -1), (1001, -1, -1)]
#     endPoints = []
#
#     # slope
...
#         # if the is a slope
#         if m != "Null":
#             # left vertical line y instersection pt
#             y1 = (m * rectCoordinates[i][0]) + b
#
#             if (rectCoordinates[i][1] <= y1 and y1 <= rectCoordinates[i][1] + rectHeight):
#                 distances[0] = (np.sqrt((rectCoordinates[i][0] - x)**2 + (y1 - y)**2), rectCoordinates[i][0], y1)
#
#             # right vertical line y intersection pt
#             y2 = (m * (rectCoordinates[i][0] + rectWidth)) + b
#
#             if (rectCoordinates[i][1] <= y2 and y2 <= rectCoordinates[i][1] + rectHeight):
#                 distances[1] = (np.sqrt((rectCoordinates[i][0] + rectWidth - x)**2 + (y2 - y)**2), rectCoordinates[i][0] + rectWidth, y2)
#
#             # bottom horizontal line x intersection pt
#             if m != 0:
#                 x1 = (rectCoordinates[i][1] - b)/m
#
#                 if (rectCoordinates[i][0] <= x1 and x1 <= rectCoordinates[i][0] + rectWidth):
#                     distances[2] = (np.sqrt((x1 - x)**2 + (rectCoordinates[i][1] - y)**2), x1, rectCoordinates[i][1])
#
#             # top horizontal line x intersection pt
#             if m != 0:
#                 x2 = ((rectCoordinates[i][1] + rectHeight) - b)/m
#
#                 if (rectCoordinates[i][0] <= x2 and x2 <= rectCoordinates[i][0] + rectWidth):
#                     distances[3] = (np.sqrt((x2 - x)**2 + (rectCoordinates[i][1] + rectHeight - y)**2), x2, rectCoordinates[i][1] + rectHeight)
#         else:
#             # make calculations using only the x coordinate
#             if (rectCoordinates[i][0] <= x and x <= rectCoordinates[i][0] + rectWidth):
#                 distances[0] = (np.absolute(rectCoordinates[i][1] - y), x, rectCoordinates[i][1])
#                 distances[1] = (np.absolute((rectCoordinates[i][1] + rectHeight) - y), x, rectCoordinates[i][1] + rectHeight)
#
#         # find the shortest distance and store the index
#         shortest = 1001
#         coord = (-1, -1)
